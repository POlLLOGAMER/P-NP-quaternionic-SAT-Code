...
3. Invarianza topológica bajo Keccak y Relajación Dinámica
   --------------------------------------------------------
   El solver ya no minimiza el error cuadrático de coordenadas ni
   se limita a un voto mayoritario estático (que al evaluar cláusulas
   de forma aislada producía "INCONCLUSIVE").
   En cambio:
   - FASE 1 (Consenso Inicial): Evalúa los signos de orientación de todos
     los literales respecto a sus hiperplanos H_c y obtiene el consenso
     geométrico mayoritario.
   - FASE 2 (Relajación de Orientación Keccak-Permutada): Permuta el orden
     de recorrido de las cláusulas usando permutaciones deterministas
     derivadas de Keccak-f[1600] y, ante cláusulas insatisfechas, aplica
     reflexiones geométricas de orientación guiadas por la proyección
     `orient(v_x, H_c)` hasta encontrar la intersección global (SAT).

4. Salida booleana pura
   ---------------------
   `assignment[v]` es el producto de las evaluaciones y reflexiones de
   orientación sobre los hiperplanos de las cláusulas. Ningún bit proviene
   de redondear un flotante ni de resolver una ecuación algebraica x = y.
...
# ============================================================
# Motor de decisión por invarianza topológica y orientación
# ============================================================
...
def oriented_quaternionic_candidate(
    ...
    keccak_rounds: int = 8,
    seed: int = 0xC0FFEE,
    max_dynamic_sweeps: int = 1000,
) -> QuaternionicResult:
    """
    Motor de decisión por predicados de orientación con invarianza
    topológica bajo permutaciones Keccak y relajación dinámica
    sobre hiperplanos de restricción.
    """
    ...
    # ------------------------------------------------------------
    # FASE 1: Consenso de Predicados de Orientación (Hiperplanos H_c)
    # ------------------------------------------------------------
    hyperplanes = [build_clause_hyperplane(row) for row in matrix]
    
    vote_pos: Dict[int, int] = {}
    vote_neg: Dict[int, int] = {}
    u_vector: Dict[int, float] = {v: 0.0 for v in range(1, nvars + 1)}
    ...
    # ------------------------------------------------------------
    # FASE 2: Relajación de Orientación Keccak-Permutada (Dinámica)
    # ------------------------------------------------------------
    # Para superar el estancamiento del voto estático local (que evaluaba
    # cláusulas de forma aislada sin propagar conflictos), reordenamos
    # topológicamente las cláusulas vía permutaciones deterministas
    # Keccak y reflejamos las orientaciones sobre los hiperplanos H_c.
    var_to_clauses: Dict[int, List[int]] = {}
    for c_idx, clause in enumerate(cnf):
        for lit in clause:
            var_to_clauses.setdefault(abs(lit), []).append(c_idx)

    rng = random.Random(seed ^ 0x9E3779B9)
    total_sweeps = max(keccak_rounds * 50, max_dynamic_sweeps)

    for sweep in range(total_sweeps):
        perm = keccak_permutation(ncl, seed=seed + sweep)
        for p in perm:
            c_idx = p - 1
            clause = cnf[c_idx]
            if not clause_satisfied(clause, assignment):
                row = matrix[c_idx]
                plane = hyperplanes[c_idx]

                # Evaluar el costo de orientación y perturbación de cada literal
                best_d = None
                min_breaks = sys.maxsize
                candidates: List[EncodedDecision] = []

                for d in row:
                    v = d.variable
                    breaks = 0
                    for check_idx in var_to_clauses.get(v, []):
                        if check_idx == c_idx:
                            continue
                        # Si la cláusula vecina está satisfecha únicamente por v,
                        # cambiar la orientación de v la dejaría insatisfecha.
                        if clause_satisfied(cnf[check_idx], assignment):
                            sat_lits = sum(1 for l in cnf[check_idx] if literal_value(l, assignment))
                            if sat_lits == 1 and not literal_value(d.literal, assignment):
                                for l_check in cnf[check_idx]:
                                    if abs(l_check) == v and literal_value(l_check, assignment):
                                        breaks += 1
                                        break
                ...
                # Selección guiada por predicado geométrico o perturbación pseudoaleatoria
                if rng.random() < 0.35:
                    best_d = rng.choice(row) if row else None
                else:
                    # Seleccionar el literal con mayor proyección/alineación de orientación
                    # geométrica sobre el hiperplano H_c (`orient_float`).
                    if candidates:
                        best_d = max(
                            candidates,
                            key=lambda d: orient_float(d, plane) if d.literal > 0 else -orient_float(d, plane)
                        )
                    elif row:
                        best_d = row[0]
                ...
                if best_d is not None:
                    v = best_d.variable
                    target_sign = +1 if best_d.literal > 0 else -1
                    u_vector[v] = target_sign * (abs(u_vector[v]) + 1.0)
                    assignment[v] = target_sign > 0
        ...
        if not failed:
            return QuaternionicResult(
                ...
                rows=len(matrix),
                columns=max((len(r) for r in matrix), default=0),
                keccak_rounds=keccak_rounds + (sweep + 1),
                elapsed_seconds=time.perf_counter() - started,
            )
    ...
    return QuaternionicResult(
        ...
        rows=len(matrix),
        columns=max((len(r) for r in matrix), default=0),
        keccak_rounds=keccak_rounds + total_sweeps,
        elapsed_seconds=time.perf_counter() - started,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Motor SAT experimental basado en predicados de orientación y relajación dinámica"
    )
    ...
    result = oriented_quaternionic_candidate(
        ...
        max_dynamic_sweeps=args.max_sweeps,
    )
    ...
    # ============================================================
    with open("ultimo_resultado.pkl", "wb") as f:
        pickle.dump(result, f)

    print("\n[!] Resultado guardado en 'ultimo_resultado.pkl'")
